...
//API for adding farmer's posts
app.post("/farmers/me/post", authMiddleware, async (req, res) => {
  try {
    const newpost = {
      product: req.body.product,
      harvestDate: req.body.harvestDate,
      acresGrown: req.body.acresGrown,
    };
    const farmer = await Farmer.findById(req.user.id);
    farmer.posts.push(newpost);
    await farmer.save();
    res.json(farmer);
  } catch (err) {
...
// API for deleting a farmer's post
app.delete(
  "/farmers/:farmerId/posts/:postId",
  authMiddleware,
  async (req, res) => {
    try {
      const { farmerId, postId } = req.params;
      ...
      const postIndex = farmer.posts.findIndex(
      ...
      farmer.posts.splice(postIndex, 1);
...
// Endpoint to get all posts for a specific farmer
app.get('/farmer/:farmerId/posts', async (req, res) => {
  // console.log(req.params.farmerId);
  const farmerId = req.params.farmerId;
  let posts=[];
  // Fetch all posts associated with the farmerId
  const farmer = await Farmer.findById(farmerId);
  if (farmer)  posts = farmer.posts;
  res.json(posts);
});
